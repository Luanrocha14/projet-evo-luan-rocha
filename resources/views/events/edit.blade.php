    ...
    <form action="{{ route('events.update', $event->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="checkbox" name="items[]" value="Cadeiras"
                {{ in_array('Cadeiras', $event->items ?? []) ? 'checked' : '' }}> Cadeiras<br>
            <input type="checkbox" name="items[]" value="Palco"
                {{ in_array('Palco', $event->items ?? []) ? 'checked' : '' }}> Palco<br>
            <input type="checkbox" name="items[]" value="Cerveja grátis"
                {{ in_array('Cerveja grátis', $event->items ?? []) ? 'checked' : '' }}> Cerveja grátis<br>
            <input type="checkbox" name="items[]" value="Open Food"
                {{ in_array('Open Food', $event->items ?? []) ? 'checked' : '' }}> Open Food<br>
            <input type="checkbox" name="items[]" value="Brindes"
                {{ in_array('Brindes', $event->items ?? []) ? 'checked' : '' }}> Brindes<br>
        </div>
